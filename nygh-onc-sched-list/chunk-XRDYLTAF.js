...
var PreferencesService = class _PreferencesService {
  ...
  columnWidths = computed(() => this._preferences().column_widths, ...ngDevMode ? [{ debugName: "columnWidths" }] : (
    /* istanbul ignore next */
    []
  ));
  loadPreferences(prsnlId) {
    ...
    try {
      ...
      if (cached) {
        ...
        this._preferences.set(this.reconcile(__spreadValues(__spreadValues({}, DEFAULT_PREFERENCES), parsed)));
      }
    } catch {
    }
  ...
  setColumnVisibility(key, visible) {
    this._preferences.update((p) => __spreadProps(__spreadValues({}, p), {
      column_visibility: __spreadProps(__spreadValues({}, p.column_visibility), { [key]: visible })
    }));
    this.savePreferences();
  }
  resetColumnVisibility() {
    this._preferences.update((p) => __spreadProps(__spreadValues({}, p), { column_visibility: {} }));
    this.savePreferences();
  }
  setColumnWidth(key, width) {
    this._preferences.update((p) => __spreadProps(__spreadValues({}, p), {
      column_widths: __spreadProps(__spreadValues({}, p.column_widths), { [key]: width })
    }));
    this.savePreferences();
  }
  savePreferences() {
  ...
  /**
   * Reconcile persisted preferences with the current column set so users who saved
   * prefs before a new column was introduced still see it (appended to the end).
   */
  reconcile(prefs) {
    const known = new Set(COLUMN_DEFINITIONS.map((c) => c.key));
    const existing = prefs.column_order.filter((k) => known.has(k));
    const existingSet = new Set(existing);
    const missing = COLUMN_DEFINITIONS.map((c) => c.key).filter((k) => !existingSet.has(k));
    const column_visibility = Object.fromEntries(Object.entries(prefs.column_visibility ?? {}).filter(([k]) => known.has(k)));
    const column_widths = Object.fromEntries(Object.entries(prefs.column_widths ?? {}).filter(([k]) => known.has(k)));
    return __spreadProps(__spreadValues({}, prefs), { column_order: [...existing, ...missing], column_visibility, column_widths });
  }
...
var ScheduleService = class _ScheduleService {
  ...
  refreshIntervalSeconds = computed(() => {
    const configured = this.configService.configuration()?.REFRESH_INTERVAL_SECONDS;
    if (configured === 0)
      return 0;
    if (typeof configured !== "number" || !Number.isFinite(configured)) {
      return DEFAULT_REFRESH_INTERVAL_SECONDS;
    }
    return Math.min(MAX_REFRESH_INTERVAL_SECONDS, Math.max(MIN_REFRESH_INTERVAL_SECONDS, Math.floor(configured)));
  }, ...ngDevMode ? [{ debugName: "refreshIntervalSeconds" }] : (
    /* istanbul ignore next */
    []
  ));
  autoRefreshEffect = effect(() => {
  ...
  }, ...ngDevMode ? [{ debugName: "autoRefreshEffect" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  _labEventSetName = signal("", ...ngDevMode ? [{ debugName: "_labEventSetName" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  labEventSetName = this._labEventSetName.asReadonly();
  ...
  loadAppointments() {
    if (!this.accessControlService.hasScheduleAccess()) {
      ...
      this._physicians.set([]);
      this._loading.set(false);
      return;
    }
    const config = this.configService.configuration();
    const apptTypeCds = this.accessControlService.resolvedAppointmentTypes();
    const chartLinkEncntrTypeCd = config?.CHART_LINK_ENCOUNTER_TYPE_CD ?? 0;
    const labConfig = this.accessControlService.resolvedLabConfig();
      ...
      customScript: {
        script: [{
          ...
          parameters: {
            ...
            chartLinkEncntrTypeCd: chartLinkEncntrTypeCd > 0 ? String(chartLinkEncntrTypeCd) : "",
            labEventSetCd: labConfig ? String(labConfig.event_set_cd) : "",
            labLookbackHours: labConfig ? String(labConfig.lookback_hours) : "",
            apptStateMeanings: (config?.APPT_STATE_MEANINGS ?? []).join(","),
            chairResourceNamePattern: config?.CHAIR_RESOURCE_NAME_PATTERN ?? "",
            activeEncntrTypeCds: (config?.ACTIVE_ENCNTR_TYPE_CDS ?? []).join(",")
          }
        }],
      ...
      try {
        ...
        if ((response?.statusData?.status ?? response?.statusdata?.status) === "S") {
          const appointments = (response.data?.appointments ?? []).map((a) => ({
            ...
            chair_resource: a.chairResource ?? a.chair_resource ?? "",
            ...
            labs: (a.labs ?? []).map((l) => ({
              ...
              event_name: l.eventName ?? l.event_name ?? "",
              event_dt_tm: l.eventDtTm ?? l.event_dt_tm ?? "",
              result_value: l.resultValue ?? l.result_value ?? "",
              result_units: l.resultUnits ?? l.result_units ?? "",
              result_status: l.resultStatus ?? l.result_status ?? "",
              normalcy: l.normalcy ?? "",
              performed_by: l.performedBy ?? l.performed_by ?? ""
            })),
            lab_orders: (a.labOrders ?? a.lab_orders ?? []).map((o) => ({
              order_id: o.orderId ?? o.order_id ?? 0,
              order_mnemonic: o.orderMnemonic ?? o.order_mnemonic ?? "",
              order_dt_tm: o.orderDtTm ?? o.order_dt_tm ?? "",
              order_status: o.orderStatus ?? o.order_status ?? "",
              dept_status: o.deptStatus ?? o.dept_status ?? "",
              performed_by: o.performedBy ?? o.performed_by ?? ""
            }))
          }));
          this._appointments.set(appointments);
          this.extractPhysicians(appointments);
          this._labEventSetName.set(response.data?.labEventSetName ?? response.data?.lab_event_set_name ?? "");
          this._lastUpdated.set(this.formatLastUpdated(/* @__PURE__ */ new Date()));
        } else {
  ...
  extractPhysicians(appointments) {
    ...
    const selectedId = this.preferencesService.selectedPhysicianId();
    if (selectedId !== null && !seen.has(selectedId)) {
      this.preferencesService.setPhysicianFilter(null);
    }
  }
  ...
  formatLastUpdated(date) {
    return `${this.formatDateForDisplay(date)} ${date.toLocaleTimeString()}`;
  }
...
//# sourceMappingURL=chunk-XRDYLTAF.js.map
