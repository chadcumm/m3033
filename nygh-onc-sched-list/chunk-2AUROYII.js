...
var AccessControlService = class _AccessControlService {
  ...
  roleHiddenColumns = computed(() => {
    const rules = this.configService.configuration()?.COLUMN_ROLE_DEFAULTS ?? [];
    const position = this.userPosition();
    const hidden = /* @__PURE__ */ new Set();
    for (const rule of rules) {
      if (rule.positions.includes(position)) {
        for (const col of rule.hidden_columns)
          hidden.add(col);
      }
    }
    return hidden;
  ...
  canAccessTab(tabKey) {
    ...
    if (this.isSuperuser())
      return true;
    if (ALL_TABS.find((t) => t.key === tabKey)?.superuserOnly)
      return false;
...
//# sourceMappingURL=chunk-2AUROYII.js.map
