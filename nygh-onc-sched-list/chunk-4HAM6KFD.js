...
var ScheduleService = class _ScheduleService {
  ...
  _selectedDate = signal(this.formatDateForDisplay(/* @__PURE__ */ new Date()), ...ngDevMode ? [{ debugName: "_selectedDate" }] : []);
  ...
  filteredAppointments = computed(() => {
    ...
    if (physicianId === null)
      return all;
    return all.filter((a) => a.attending_physician_id === physicianId);
  ...
  isToday = computed(() => {
    return this._selectedDate() === this.formatDateForDisplay(/* @__PURE__ */ new Date());
  ...
  loadAppointments() {
    ...
    this.customService.load({
        ...
        script: [{
          ...
          parameters: {
            ...
            requestData: JSON.stringify({
              schedule_date: this.formatDateForCCL(this._selectedDate()),
              appt_type_cds: apptTypeCds
            })
          }
        }],
      ...
      try {
        ...
        if (response?.statusData?.status === "S") {
          const appointments = (response.data?.appointments ?? []).map((a) => ({
            person_id: a.person_id ?? a.PERSON_ID ?? 0,
            encntr_id: a.encntr_id ?? a.ENCNTR_ID ?? 0,
            sch_event_id: a.sch_event_id ?? a.SCH_EVENT_ID ?? 0,
            patient_name: a.patient_name ?? a.PATIENT_NAME ?? "",
            patient_location: a.patient_location ?? a.PATIENT_LOCATION ?? "",
            encounter_type: a.encounter_type ?? a.ENCOUNTER_TYPE ?? "",
            attending_physician: a.attending_physician ?? a.ATTENDING_PHYSICIAN ?? "",
            attending_physician_id: a.attending_physician_id ?? a.ATTENDING_PHYSICIAN_ID ?? 0,
            appt_type: a.appt_type ?? a.APPT_TYPE ?? "",
            appt_status: a.appt_status ?? a.APPT_STATUS ?? "",
            requested_date: a.requested_date ?? a.REQUESTED_DATE ?? "",
            day_of_treatment: a.day_of_treatment ?? a.DAY_OF_TREATMENT ?? "",
            ordered_treatment: a.ordered_treatment ?? a.ORDERED_TREATMENT ?? "",
            special_instructions: a.special_instructions ?? a.SPECIAL_INSTRUCTIONS ?? ""
          }));
  ...
  goToToday() {
    this._selectedDate.set(this.formatDateForDisplay(/* @__PURE__ */ new Date()));
  ...
  navigateDate(days) {
    const current = this.parseDateFromDisplay(this._selectedDate());
    current.setDate(current.getDate() + days);
    this._selectedDate.set(this.formatDateForDisplay(current));
    this.loadAppointments();
    this.resetAutoRefresh();
  }
  resetAutoRefresh() {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ScheduleListComponent, { className: "ScheduleListComponent", filePath: "src/app/components/schedule-list/schedule-list.ts", lineNumber: 13 });
})();
export {
  ScheduleListComponent
};
//# sourceMappingURL=chunk-4HAM6KFD.js.map
